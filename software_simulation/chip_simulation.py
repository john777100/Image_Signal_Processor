...
def get_raw_data(file_name):
    ...
    return im
def mosaic_jpg(file_name = IMAGE_FILE):
	im = imageio.imread(file_name)
	print("Loaded %s" % (file_name))
	print("Shape of image",im.shape)
	print("Max of image",np.maximum(im))
	print("Min of image",np.minimum(im))
	return im
...
def whitebalance(image, input_type = 'rgb', k = 'dynamic'):
    ...
    if(input_type == 'bayer'):
        ...
        image[:,:,2] *= k_b
        return np.where(image>255, 255, image)
def gamma(rgb_image, gamma=2.2, mode = 'bundle'):
...
def output_image(rgb_image, in_max = 255, stage='demosaic', comment=''):
    assert stage in ['raw','sharpen','demosaic', 'white', 'gamma', 'denoise','pure_raw']
...
def print_stage_string(stage='demosaic', comment=''):
    assert stage in ['raw','sharpen','demosaic', 'white', 'gamma', 'denoise']
